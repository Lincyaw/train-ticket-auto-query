...
class TestTravelService(unittest.TestCase):
    def setUp(self):
        ...
        self.headerss = {'Authorization': f'Bearer {token}'}
    ...
    def test_get_train_type_by_trip_id(self):
        ...
        train_type = get_train_type_by_trip_id(self.client, trip_id, self.host, headers)
        print(train_type)
    ...
    def test_get_trips_by_route_id(self):
        ...
        trips = get_trips_by_route_id(self.client, route_ids, self.host, headers)
    ...
    def test_get_trip_all_detail_info(self):
        gtdi = TripAllDetailInfo(tripId=str(fake.uuid4()),
                                 ...
                                 to=str(fake.city()))
        detail_info = get_trip_all_detail_info(self.client, gtdi, self.host, headers)
    ...
    def test_end_to_end(self):
        ...
        route_ids = TravelInfo(loginId=str(fake.uuid4()),
                               ...
                               startTime=str(fake.date_time()),
                               endTime=str(fake.date_time()))
        create_response = create_trip(self.client, route_ids, self.host, headers)
        self.assertIsInstance(create_response, str)

        # 获取创建的Trip
        created_trip = retrieve_trip(self.client, str(fake.uuid4()), self.host, headers)
        self.assertIsInstance(created_trip, ReturnBody)

        # 获取TrainType
        train_type = get_train_type_by_trip_id(self.client, str(fake.uuid4()), self.host, headers)
        self.assertIsInstance(train_type, ReturnBody)

        # 获取Route
        route = get_route_by_trip_id(self.client, str(fake.uuid4()), self.host, headers)
        self.assertIsInstance(route, ReturnBody)
        ...
        # 并行查询Trip
        queried_parallel_trips = query_trips_in_parallel(self.client, query_info, self.host, headers)
        ...
        detail_info = TripAllDetailInfo(tripId=str(fake.uuid4()),
                                 ...
                                 from_location=str(fake.city()),
                                 to=str(fake.city()))
        trip_detail = get_trip_all_detail_info(self.client, detail_info, self.host, headers)
        self.assertIsInstance(trip_detail, dict)

        # 删除Trip
        delete_response = delete_trip(self.client, str(fake.uuid4()), self.host, headers)
